...
*/
function expectablePromise/*::<T>*/(originalPromise/* : Promise<T> */)/*: ExpectablePromise<T> */ {
...
function createSuspendingComponent() {
    let _resolve, _reject,
        _resolveAndRethrow, _rejectAndRethrow,
        suspension;
    ...
    return [SuspendingClazz, SuspendingFunctional, { resolve, resolveAndRethrow, reject, rejectAndRethrow }]
}
...
describe("prepass", () => {
    ...
    describe("vnode traversal", () => {
        ...
        it("should traverse Fragments", async () => {
            ...
            await prepass(<Fragment><Inner /></Fragment>);
        ...
        it("should traverse rendered children", async () => {
            ...
            await prepass(<Outer><Inner /></Outer>);
        ...
        it("should traverse children rendered in nested vnode tree", async () => {
            const Inner = jest.fn(() => <div/>);
            const Outer = jest.fn(({ children }) => <div><Fragment>{children}</Fragment></div>);
    ...
    describe("suspensions", () => {
        ...
        it("should await suspension of class component", async () => {
            const [Suspending, _, { resolve, resolveAndRethrow }] = createSuspendingComponent();

            const spy = jest.spyOn(Suspending.prototype, 'render');

            const promise = expectablePromise(prepass(
                <div>
                    <Suspending foo="bar" />
                </div>
            ));
            ...
            resolveAndRethrow();
            await tick();
            expect(spy.mock.calls).toEqual([[{foo:'bar'},{},{}],[{foo:'bar'},{},{}]]);
            ...
            await tick();
            await promise;
            expect(spy.mock.calls).toEqual([[{foo:'bar'},{},{}],[{foo:'bar'},{},{}],[{foo:'bar'},{},{}]]);
        });

        it("should await suspension of functional component", async () => {
            const [_, _Suspending, { resolve, resolveAndRethrow }] = createSuspendingComponent();
            ...
            resolveAndRethrow();
            await tick();
            expect(Suspending.mock.calls).toEqual([[{foo:'bar'},{}],[{foo:'bar'},{}]]);
            ...
            await tick();
            await promise;
            expect(Suspending.mock.calls).toEqual([[{foo:'bar'},{}],[{foo:'bar'},{}],[{foo:'bar'},{}]]);
        });
        ...
        it("should await throwing suspension", async () => {
            const [_, _Suspending, { reject, rejectAndRethrow }] = createSuspendingComponent();
    ...
    describe("error handling", () => {
        it("should reject when non-promise errors are thrown from functional components", async () => {
            const MyComp = () => { throw new Error('hello'); };
    ...
    describe("visitor", () => {
        ...
        it("should call the visitor on nested components", async () => {
            ...
            expect(visitor.mock.calls).toEqual([[myClassComp, myClassComp.__c],[myFuncComp, undefined]]);
        });
        ...
        it("should await suspensions of the visitor", async () => {
            let resolve;
            const visitor = jest.fn(() => new Promise((res) => { resolve = res }));
            const myComp = <MyFuncComp />
